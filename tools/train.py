...
from enseg.utils import wait_for_allow_execution
...
def main():
    ...
    if args.config.find("DEBUG") != -1:
        args.config = "/home/wzx/weizhixiang/ensegment/configs/nightlab_mae/convnext_h256w512_bs8_large_21k_fulltrain_80k_deformable.py"
    ...
    meta = dict()
    meta["gpu_number"] = torch.cuda.device_count()
    logger.info("Gpu numbers:{}".format(meta["gpu_number"]))
    meta["total_batch_size"] = cfg["data"]["samples_per_gpu"] * meta["gpu_number"]
    logger.info("Total numbers:{}".format(meta["total_batch_size"]))
    # log env info
    ...
    if distributed and cfg.get("runner", None):
        ...
        if isinstance(cfg.optimizer, Iterable):
            optimizer = build_optimizers(network, cfg.optimizer)  # Custom optimizer
        else:
            optimizer = build_optimizer(network, cfg.optimizer)  # Custom optimizer
