...
network = dict(
    ...
    backbone=dict(img_size=(256, 512), drop_rate=0.0),
    seg=dict(
        num_convs=4,
        up_scale=2,
        kernel_size=3,
        loss_decode=[
            dict(type="CrossEntropyLoss", use_sigmoid=False, loss_weight=0.01),
            dict(type="NuclearLoss", patch_size=16, use_softmax=True, loss_weight=0.1),
        ],
    ),
    gen=dict(
        type="ViTGen",
...
network = dict(
    type='SegMAEfp16',
    pretrained=
    '/home/wzx/weizhixiang/ensegment/pretrain/vit/mmlab/jx_vit_large_p16_224-4ee7a4dc.pth',
    backbone=dict(
