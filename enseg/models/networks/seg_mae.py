...
class SegMAE(BaseNetwork):
    ...
    def train_step(self, data_batch, optimizer, ddp_reducer=None, **kwargs):
        ...
        self._optim_zero(optimizer, "seg", "aux", "backbone", "gen", "neck")
        if "1" in work:
            """
            input: img: (N,C,H,W)
        ...
        if "M" in work:
            ...
            losses_rec, vars_rec = self.masker.decode(self.gen, criterion, encode_infos)
        ...
        total_loss.backward()
        self._optim_step(optimizer, "seg", "aux", "backbone", "gen", "neck")
