...
class EnsegV5(BaseNetwork):
    ...
    def forward_gen_train(self, dataA, dataB):
        ...
        fake_img = self.gen(self.feature["A"], dataA["img"], norm_cfg)
        rec_img = self.gen(self.feature["B"], dataB["img"], norm_cfg)
        if not self.keep_size:
            ...
            self.ref_img = dict(A=dataA["img"], B=dataB["img"])
        loss_rec = self.creterion_rec(rec_img, self.ref_img["B"], norm_cfg)
        loss_adv = self.gan_loss(self.dis(fake_img), target_is_real=True, is_disc=False)
        losses = {
            "gen.loss_adv": loss_adv,
            "gen.loss_idt": loss_rec,
        }
        outputs = {
            "gen/realA": self.ref_img["A"],
            "gen/fakeB": fake_img.detach(),
            "gen/realB": self.ref_img["B"],
    ...
    def forward_dis_train(self, dataA, dataB, generated):
        ...
        pred_fake = self.dis(fake_img)
        pred_real = self.dis(real_img)
        losses = dict()
    ...
    def train_step(self, data_batch, optimizer, ddp_reducer=None, **kwargs):
        ...
        dataB = data_batch[1]
        self.feature = dict(
            A=self.backbones["A"](dataA["img"]), B=self.backbones["B"](dataB["img"])
        )
