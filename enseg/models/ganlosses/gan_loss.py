...
class GANLoss(nn.Module):
    ...
    @torch.no_grad()
    def get_accuracy(self, real: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
        if self.gan_type in ["vanilla", "lsgan"]:
            return ((real > 0.5).sum() + (fake < 0.5).sum()) / (
                real.numel() + fake.numel()
            )
        else:
            return (real > fake).sum() / real.numel()
