...
from ..builder import DECODE_SEG
...
CONV_LAYERS.register_module("deformable_conv", module=DeformConv2dPack)
...
class UPerHead(BaseDecodeHead):
    ...
    def psp_forward(self, inputs):
        ...
        x = inputs[-1]
        psp_outs = [x]
        psp_outs.extend(self.psp_modules(x.contiguous()))
        psp_outs = torch.cat(psp_outs, dim=1)
        output = self.bottleneck(psp_outs.contiguous())
    ...
    def forward(self, inputs):
        ...
        # build laterals
        laterals = [
            lateral_conv(inputs[i].contiguous())
