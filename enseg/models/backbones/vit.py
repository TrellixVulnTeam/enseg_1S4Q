...
class VisionTransformer(BaseModule):
    ...
    def init_weights(self):
        if (
            ...
            logger.info("load vit from {}".format(self.init_cfg["checkpoint"]))
    ...
    def embed_mae(self, inputs: torch.Tensor) -> torch.Tensor:
        B = inputs.shape[0]
        ...
        x = x[:, 1:, :]
        return x
    def forward_mae(self, x: torch.Tensor) -> torch.Tensor:
